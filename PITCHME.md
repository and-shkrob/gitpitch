...
```java
    given(sherbook())
            ...
            .body(is("OK"))
```
@[1](Я хочу сделать запрос в шербук)
@[2](Хочу получить статус. Отправляю GET на адрес /status)
@[3](Запустить запрос и затем ...)
@[4](Проверить, что запрос успешно вернул статус 200)
@[5](Проверить, что шербук вернул слово OK)
@snapend
