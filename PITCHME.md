...
@[3](Я хочу сделать запрос в шербук)
@[4](Хочу получить статус. Отправляю GET на адрес /status)
@[5](Запустить запрос и затем ...)
@[6](Проверить, что запрос успешно вернул статус 200)
@[7](Проверить, что шербук вернул слово OK)
